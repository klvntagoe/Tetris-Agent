...
def _preProcessCNNInput1(input, device):
    ...
    return torch.tensor(np.array([board, mask, queue, holder]), device=device, dtype=torch.float).unsqueeze(0)

class Q_CNN_1(nn.Module):
    NAME = "QNetwork_cnn1"
    device = torch.device(
    ...
    def preProcess(input):
        return _preProcessCNNInput1(input=input, device=Q_CNN_1.device)

class Q_CNN_2(nn.Module):
    NAME = "QNetwork_cnn2"
    # todo make all device params member vars
    device = torch.device(
        "cuda" if torch.cuda.is_available() 
        else "mps" if torch.backends.mps.is_available() 
        else "cpu")
    ...
    def __init__(self,numOutputs):
        ...
        self.network = nn.Sequential(
        # Feature extraction block 1
            nn.Conv2d(in_channels=4, out_channels=32, kernel_size=(4,4), stride=2, padding=2, groups=4), # depth-wise separated convolution over each channel
            nn.MaxPool2d(kernel_size=(2,2)),
            nn.GroupNorm(num_groups=4, num_channels=32),  # noamlize channels independently
            nn.ReLU(),
        # Feature extraction block 2
    ...
    def preProcess(input):
        return _preProcessCNNInput1(input=input, device=Q_CNN_2.device)